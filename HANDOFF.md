...
### Michael
- **THE LAUNCH BLOCKER: put the Read/Write WooCommerce key into Vercel.** The key
  in `WC code.rtf` (dated 14 Aug) has write access and is now in `.env.local`; the
  one that was there before did not, and `PUT /products/{id}` returned
  `401 "The API key provided does not have write permissions."` Vercel's copies are
  typed **Sensitive**, so their values cannot be read back even by `vercel env
  pull` - all that is visible is that they were created 21 days ago, before this
  key existed. **Assume they are the old read-only pair and re-set them**; it is
  idempotent if they are already right. Also check `WC_WRITE_ENABLED`, which must
  be `true` in production. Both have to be right: a Read/Write key with the flag
  off writes nothing, and the flag on with a read-only key 401s. **If this is
  wrong at launch, customers pay through Stripe for orders that never reach
  WooCommerce and so never reach Unleashed.** A redeploy is required after any env
  change.
...
- **Interparcel** want 10-20 example shipments before they will quote rates.
  **Done** - 15 real consignments in `reports/interparcel-sample-shipments.csv`,
  generated by `npm run report:freight` from real catalogue weights and cartons,
  built by the production parcel rule. They run 1kg to 450kg across metro,
  regional, remote and Tasmania, and include multi-carton orders, because 65 of
  our 186 quotable products are over 30kg and a small-parcel rate card would not
  cover us. **Still needs the despatch warehouse address before sending** - every
  row lacks an origin, and it is the same missing `FREIGHT_COLLECTION_*` value
  that blocks live quoting.
- **28 products have specs that genuinely disagree** between the two content
  ...
  out of the Interparcel samples: we will not quote a rate against a number we
  cannot stand behind.
